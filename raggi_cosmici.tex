...
\begin{frame}
  ...
  \item che il rapporto positroni/elettroni è compatibile con l'ipotesi di
    isotropia
  \end{enumerate}
  \uncover<+->{Questi dati non mostrano molto di nuovo, sono in gran parte una
    \alert{conferma di quanto già trovato da PAMELA}
    (\doi{10.1038/nature07942}), sebbene con una precisione migliore e in un
    intervallo di energia più grande}
\end{frame}
...
\begin{frame}
  ...
  \item<+-> annichilazione di materia oscura
    ...
    \item[\con] massa dedotta della particella di materia oscura
      ($\gtrsim \SI{350}{\giga\electronvolt\per\clight\squared}$) è
      incompatibile con risultati di altri esperimenti (CDMS:
