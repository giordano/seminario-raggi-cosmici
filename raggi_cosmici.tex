...
\begin{frame}
  ...
  \item<+-> materia oscura
    ...
    \item[\con] distribuzione isotropa
      % Qualcuno (arXiv:1304.1791) sostiene che la decrescita dovrebbe esserci
      % anche nel caso di ipotesi di sorgente pulsar, per perdite di energia e/o
      % spettro intrinseco di iniezione della sorgente, quindi la presenza della
      % decrescita potrebbe non essere determinante per discriminare fra i due
      % modelli.
  ...
  \uncover<+->{In ogni caso,
    ...
    né in un senso né nell'altro, bisognerà attendere ulteriori dati per poter
    trarre conclusioni più concrete}
\end{frame}
